# Make sure we have ansible_collections/netapp/trident_protect as a prefix. This is
...
err_msg := Place collection at <WHATEVER>/ansible_collections/netapp/trident_protect
ifeq (true,$(CI))
  $(info Running in CI setting, skipping directory checks.)
else ifneq (trident_protect,$(collection))
  $(error $(err_msg))
else ifneq (netapp,$(namespace))
  $(error $(err_msg))
